<template>
  <div class="page-container">
    <h2 class="page-title">武器查询</h2>
    
    <div class="search-box">
      <el-input
        v-model="keyword"
        placeholder="输入武器名称或武器ID搜索..."
        @keyup.enter="search"
        clearable
    ...
    <el-empty v-else-if="searched" description="未找到相关武器" />

    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="武器详情" width="600px">
      <div v-if="currentItem" class="detail-content">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="名称">{{ currentItem.name }}</el-descriptions-item>
          <el-descriptions-item label="类型">{{ currentItem.weaponType }}</el-descriptions-item>
          ...
          <el-descriptions-item label="小怪伤害">{{ currentItem.dmg_small }}</el-descriptions-item>
          <el-descriptions-item label="大怪伤害">{{ currentItem.dmg_large }}</el-descriptions-item>
          <el-descriptions-item label="射程">{{ currentItem.range }}</el-descriptions-item>
...
const search = async () => {
  ...
  try {
    const res = await api.getWeapons(keyword.value.trim(), 1, pageSize.value)
    if (res.data.success) {
...
const handlePageChange = async (page) => {
  ...
  try {
    const res = await api.getWeapons(keyword.value.trim(), page, pageSize.value)
    if (res.data.success) {
      results.value = res.data.data
    }
  } catch (error) {
